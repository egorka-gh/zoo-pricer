  ...
  export default {
    ...
    computed: {
      ...
      step () {
        ...
        if (singleStep > 0 && singleStep % step > 0) {
          //log.error('如果设置了单步滚动,step需是单步大小的约数,否则无法保证单步滚动结束的位置是否准确。~~~~~')
        }
        return step
      }
    },
    methods: {
      ...
      _move () {
        ...
        if(this.yPos % 100 == 0) this.realBoxHeight = this.$refs.realBox.offsetHeight
        ...
        this.$nextTick(() => {
          ...
          if (this.scrollSwitch) {
            ...
            setTimeout(() => {
              ...
              log.info('realBoxHeight=',this.realBoxHeight)
              this._move()
            }, 0);
          } else {
            this._cancle()
      ...
      _stopMove () {
        ...
        if (this.singleWaitTime) clearTimeout(this.singleWaitTime)
        this._cancle()
        log.info('realBoxHeight=',this.realBoxHeight, this.$refs.realBox.offsetHeight)
